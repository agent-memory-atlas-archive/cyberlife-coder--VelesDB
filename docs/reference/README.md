...
| File | Description |
|------|-------------|
| [`promise-contract.json`](./promise-contract.json) | Verbatim-pinned claims (numbers/phrasings) checked against `README.md` and other docs by `scripts/check-promise-contract.py`. Not a doc to read — a gate contract; see the warning comment at the top of the root [`README.md`](../../README.md). Each claim records its figure's source, date, machine and version; `scripts/check-figure-sources.py` refuses a performance figure in the docs or rustdoc that no claim registers, either by a part of its line or, with `"covers_section": true`, by the heading of the section it sits in. It is a heuristic: it recognizes a figure by its unit together with a keyword, a verb or a table context, so a number written with no unit and no such word, or a figure drawn in an image, passes unseen; what binds a figure to its run is the register, not the guard. |
