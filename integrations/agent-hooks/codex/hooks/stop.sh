#!/usr/bin/env bash
# Codex CLI Stop hook: continue once with the learning-loop checklist on the
# first Stop of an opted-in session and again after every later covered edit
# batch. Repositories without enforcement retain the older once-per-session
# save reminder.
#
...
#
# The edit marker is consumed before the continuation, so its next Stop passes;
# a later apply_patch creates a new marker and earns a new check. The first-Stop
...
# apply_patch can cover repositories outside cwd (and more than one at once),
